...
namespace PokeGuide.Service
{
    public class StaticDataService : DataService, IStaticDataService
    {
        ...
        public async Task<IEnumerable<Language>> LoadLanguagesAsync(int displayLanguage, CancellationToken token)
        {
            try
            {
                ...
                return list.Select(s => new Language { Id = s.Id, Name = s.Name });
            }
            catch (Exception)
            {
        ...
        public async Task<IEnumerable<GameVersion>> LoadVersionsAsync(int displayLanguage, CancellationToken token)
        {
            try
            {
                ...
                return list.Select(s => new GameVersion { Generation = s.Generation, Id = s.Id, Name = s.Name, VersionGroup = s.VersionGroupId });
            }
            catch (Exception)
            {
